...
export default function MostCommonDiseases () {
    const encabezadosEnfermedadesMenosComunes = [
        {field: 'enfermedad', text: 'Enfermedad'},
        {field: 'cantidad', text: 'Cantidad'}
    ];

    const consultarEnfermedadesMasComunes = async () => {
        try {
            const response = await axios({
                url: `${base_url}/api/Enfermedades/mas/comunes`,
                method: 'GET',
                params: {
            ...
            if (response.status === 200 && response.data.status) {
                ...
                for (const disease of response.data.data) {
                    data.push({
                        ...
                        cantidad: disease.Veces_Diagnosticada,
                    });
                }
                setListaEnfermedadesMasComunes(data);
    ...
    const cancelarFn = () => {
        setFechaFin('');
        setFechaInicio('');
        setListaEnfermedadesMasComunes([]);
        setListaLength(0)
    }
    ...
    const [listaEnfermedadesMasComunes, setListaEnfermedadesMasComunes] = useState([]);
    ...
    return <div className='modalDiv'>
        <div className="reportSearcherInputs">
            ...
            <div className="crudFormControls">
                <button className="guardarBtn" onClick={consultarEnfermedadesMasComunes}><i class="bi bi-search"></i></button>
                ...
                {listaLength > 0 && <ExportExcel 
                        excelData={listaEnfermedadesMasComunes} 
        ...
        <div className='TableModalComponent' ref={targetRef}>
            ...
            <DataTable 
                ...
                rows={listaEnfermedadesMasComunes}
                wide={true}
            />
        </div>
    </div>
